...
describe("HomeScreen", () => {
  ...
  it("renders hologram entry pods without changing the primary click targets", () => {
    ...
    expect(html).toContain('class="home-match-mode-tickets"');
    expect(html).toContain('class="home-match-mode-ticket home-match-mode-ticket-spark"');
    expect(html).toContain('class="home-match-mode-count">3</span><span class="home-match-mode-label">匹配中</span>');
  ...
  it("uses a tactical ID card and skewed navigation cards", () => {
    ...
    expect(html).toContain("utility-entry-description");
    expect(html).toContain("成员补给");
    expect(html).toContain("物资购入");
    expect(html).toContain("道具管理");
    expect(html).toContain("天梯记录");
    expect(html).toContain("当前房间");
    expect(html).toContain("社交列表");
    ...
    expect(utilityEntryBlock).toContain("min-height: 78px");
    ...
    expect(css).toContain(".home-grid-featured > .home-utility-grid .utility-entry small {\n  display: block");
    expect(utilityTextBlock).toContain("transform: skewX(15deg)");
    expect(utilityHoverBeforeBlock).toContain("animation: home-nav-flow");
    expect(brightToolboxEntry).toContain(".home-match-mode-tickets");
  ...
  it("keeps the HUD footer minimal and rewrites the mobile lobby layout", () => {
    ...
    expect(brightMobileCss).toContain("grid-template-rows: minmax(188px, clamp(210px, 58vw, 270px)) auto !important");
  ...
  it("renders the recruitment entry as an enabled home utility action with a ready background alert", () => {
    ...
    expect(source).toContain("成员补给");
    expect(source).toContain("utility-entry-description");
