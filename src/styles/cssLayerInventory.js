export const CSS_DEBT_BASELINE = {
  ...
  featureDeltas: [
    "2026-09-07: uses three bounded campus home/handbook owners, with phone rules split to keep each concrete owner below 6000 bytes, removes the activity-room label, ownership count and sortie stamp, and lays out ordinary handbook portraits above a separate name/flag-action row. Desktop columns adapt to width; portrait phones use two columns and 44px actions with outer scrollbar gutter released. Existing tokens and media families are reused; no assets, keyframes or high z-index declarations are added. The 2596-byte and 53-important-declaration increase from the prior trial is confined to these owners and preserves corrupted presentation. Metrics cover the isolated commit as well as the dirty checkout so unrelated WIP reductions are not required for this change to pass.",
