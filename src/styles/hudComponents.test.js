...
const aemeathNameplateMotionCss = normalizeCss(readFileSync(new URL("./hud-components/user-identity/aemeath-digital-flight-nameplate-motion.css", import.meta.url), "utf8"));
const semanticNameplateMotionCss = normalizeCss(readFileSync(new URL("./hud-components/user-identity/semantic-ignition-motion.css", import.meta.url), "utf8"));
const deniaNameplateCss = normalizeCss(readFileSync(new URL("./hud-components/user-identity/denia-echo-nameplate.css", import.meta.url), "utf8"));
const deniaNameplateMotionCss = normalizeCss(readFileSync(new URL("./hud-components/user-identity/denia-echo-nameplate-motion.css", import.meta.url), "utf8"));
...
describe("component-level HUD refinements", () => {
  ...
  it("keeps the Bright School shop modal centered and free from tactical bleed-through", () => {
    ...
    expect(themesCss).toContain(".app-shell.player-theme-enabled.theme-bright-school.theme-bright-school .shop-wallet");
  ...
  it("loads shared user identity cosmetics for username surfaces", () => {
    ...
    expect(hudCss).toContain("--user-nameplate-base-width: 150px");
    expect(hudCss).toContain("--user-nameplate-base-height: 32px");
    expect(hudCss).toContain("--user-nameplate-padding-left: calc(39px * var(--user-nameplate-scale))");
    expect(hudCss).toContain("--user-nameplate-padding-right: calc(18px * var(--user-nameplate-scale))");
    ...
    expect(hudCss).toContain("font-style: italic");
    expect(hudCss).toContain("animation: semantic-nameplate-carrier-flow 2.45s linear -0.15s infinite");
    expect(hudCss).toContain("animation: semantic-nameplate-ignition-flare 2.45s linear -0.15s infinite");
    expect(hudCss).toContain("animation: semantic-nameplate-rune-flow 2.45s linear -0.15s infinite");
    expect(hudCss).toContain("animation: semantic-nameplate-tail-stream-a 2.45s linear -0.15s infinite");
    expect(hudCss).toContain("animation: semantic-nameplate-tail-stream-b 2.45s linear -0.15s infinite");
    expect(hudCss).toContain("right: calc(-7px * var(--user-nameplate-scale))");
    expect(hudCss).toContain("width: calc(40px * var(--user-nameplate-scale))");
    expect(hudCss).toContain("drop-shadow(0 0 calc(3.2px * var(--user-nameplate-scale))");
    expect(hudCss).toContain("rgba(255, 244, 194, 0.92) 48%");
    expect(hudCss).toContain("56% calc(1.35px * var(--user-nameplate-scale))");
    expect(hudCss).toContain("background-size: 82% 1.2px, 64% 1px, 48% 0.9px");
    expect(hudCss).toContain("transform: translateX(196%) skewX(-9deg)");
    expect(hudCss).toContain("clip-path: polygon(0 36%, 100% 0, 82% 52%, 100% 100%, 0 68%, 16% 50%)");
    expect(hudCss).toContain(".user-identity-nameplate-effect::before");
    expect(hudCss).toContain(".user-identity-nameplate-effect::after");
    expect(semanticNameplateMotionCss).not.toContain("scale(");
    expect(semanticNameplateMotionCss).not.toContain("radial-gradient");
    expect(semanticNameplateMotionCss).not.toContain("border-radius");
    expect(hudCss).not.toContain("semantic-nameplate-sigil-breathe");
    expect(hudCss).not.toContain("semantic-nameplate-rune-pulse");
    expect(hudCss).not.toContain("semantic-nameplate-rune-drift");
    expect(hudCss).not.toContain("semantic-nameplate-tail-aura");
    expect(hudCss).not.toContain("semantic-nameplate-tail-trace");
    expect(hudCss).not.toContain("semantic-nameplate-citrus-glint");
    expect(hudCss).not.toContain("semantic-nameplate-wind-tail");
    ...
    expect(hudCss).toContain("Danya's selected off-center pearlescent dream-bubble nameplate");
    ...
    expect(deniaNameplateCss).toContain("color: #fffaf5");
    expect(deniaNameplateCss).toContain("font-family: var(--font-ui-default)");
    expect(deniaNameplateCss).toContain("font-style: italic");
    expect(deniaNameplateCss).toContain("letter-spacing: calc(0.08px * var(--user-nameplate-scale))");
    expect(deniaNameplateCss).toContain("0 1px 1px rgba(51, 28, 104, 0.96)");
    expect(hudCss).toContain("animation: denia-nameplate-pearl-sheen 4.4s ease-in-out");
    expect(hudCss).toContain("animation: denia-nameplate-soft-refraction 3.8s ease-in-out");
    expect(hudCss).toContain("animation: denia-nameplate-tail-fold 3.6s ease-in-out");
    expect(hudCss).toContain("animation: denia-nameplate-dream-glints 3.9s ease-in-out");
    expect(hudCss).toContain("rgba(232, 210, 255, 0.96)");
    expect(hudCss).toContain("rgba(255, 153, 226, 0.7)");
    expect(deniaNameplateMotionCss).toContain("from { opacity: 0.52");
    expect(deniaNameplateMotionCss).toContain("to { opacity: 0.82");
    expect(deniaNameplateMotionCss).not.toContain("scale(");
    expect(deniaNameplateMotionCss).not.toContain("rotate(");
    expect(hudCss).not.toContain("denia-nameplate-portal-breathe");
    expect(hudCss).not.toContain("denia-nameplate-inner-echo");
    expect(hudCss).toContain('[data-nameplate-id="reward-aemeath-spark-100-wins-nameplate"]');
    expect(hudCss).toContain("Aemeath's paper-flight beacon nameplate");
    expect(hudCss).toContain("--user-nameplate-padding-left: calc(34px * var(--user-nameplate-scale))");
    expect(hudCss).toContain("--user-nameplate-padding-right: calc(28px * var(--user-nameplate-scale))");
    expect(hudCss).toContain("--aemeath-nameplate-home-offset-x: calc(12px * var(--user-nameplate-scale))");
    expect(hudCss).toContain(".home-player-plaque .user-identity[data-nameplate-id=\"reward-aemeath-spark-100-wins-nameplate\"] .user-identity-name-tag");
    expect(hudCss).toContain("translate: var(--aemeath-nameplate-home-offset-x) 0");
    ...
    expect(hudCss).toContain("color: #f5ffff");
    expect(hudCss).toContain("font-family: var(--font-ui-default)");
    expect(hudCss).toContain("font-style: italic");
    expect(aemeathNameplateCss).toContain("letter-spacing: calc(0.08px * var(--user-nameplate-scale))");
    expect(aemeathNameplateCss).toContain("0 1px 1px rgba(1, 37, 68, 0.98)");
    expect(aemeathNameplateCss).toContain("0 0 3px rgba(201, 246, 255, 0.8)");
    expect(aemeathNameplateCss).not.toContain("-webkit-text-stroke");
    expect(aemeathNameplateCss).not.toContain("font-weight: 900");
    expect(hudCss).toContain("animation: aemeath-nameplate-paper-wake 2.6s linear -0.18s infinite");
    expect(hudCss).toContain("animation: aemeath-nameplate-plane-signal 2.6s linear -0.18s infinite");
    expect(hudCss).toContain("animation: aemeath-nameplate-rim-flight 2.6s linear -0.18s infinite");
    expect(hudCss).toContain("animation: aemeath-nameplate-tail-flight-a 2.6s linear -0.18s infinite");
    expect(hudCss).toContain("animation: aemeath-nameplate-tail-flight-b 2.6s linear -0.18s infinite");
    expect(hudCss).toContain("animation: aemeath-nameplate-pixel-glints 2.3s ease-in-out");
    expect(hudCss).toContain("rgba(84, 234, 255, 0.9)");
    expect(hudCss).toContain("rgba(255, 118, 204, 0.56)");
    expect(hudCss).toContain("mix-blend-mode: screen");
    expect(hudCss).toContain("54% calc(1.3px * var(--user-nameplate-scale))");
    expect(hudCss).toContain(".user-identity-nameplate-effect::before");
    expect(hudCss).toContain(".user-identity-nameplate-effect::after");
    expect(aemeathNameplateCss).not.toContain("repeating-linear-gradient");
    expect(aemeathNameplateCss).not.toContain("radial-gradient");
    expect(aemeathNameplateMotionCss).not.toContain("scale(");
    expect(aemeathNameplateMotionCss).not.toContain("rotate(");
    expect(hudCss).not.toContain("aemeath-nameplate-sortie-signal");
    expect(hudCss).not.toContain("aemeath-nameplate-snowfluff-ping");
    expect(hudCss).not.toContain("aemeath-nameplate-speed-lanes");
    expect(hudCss).not.toContain("aemeath-nameplate-pixel-beacons");
  });
  ...
  it("adds a calmer Bright School visual refinement pass", () => {
    ...
    expect(imageEntryButtonsLayer).toContain("button.practice-entry-button img");
    expect(imageEntryButtonsLayer).toContain("button.practice-entry-button:hover img");
    expect(imageEntryButtonsLayer).toContain("button.practice-entry-button:active img");
