...
function mountPixiEffect({ host, boardSize, pendingSkill, durationMs, reducedMotion, audioSettings }) {
  ...
  import("pixi.js").then(async (pixi) => {
    ...
    playEffect({ app, pixi, host, boardSize, pendingSkill, durationMs, reducedMotion });
...
function playEffect({ app, pixi, host, boardSize, pendingSkill, durationMs, reducedMotion }) {
  if (pendingSkill.effectType === "hidden-hand") {
    if (reducedMotion) {
      playReducedMotionBoardSweep({ app, pixi, host, durationMs });
      return;
    }
    playDataStreamHiddenHand({ app, pixi, host, durationMs });
    return;
  }
  ...
  if (pendingSkill.effectType === "erase-point") {
    playMeteorErase({ app, pixi, target, durationMs });
    return;
  }
  if (pendingSkill.effectType === "flip-stone") {
    playBubbleFlip({ app, pixi, target, durationMs });
    return;
  }
  if (pendingSkill.effectType === "random-blast") {
    playBaconbitsBlast({ app, pixi, host, boardSize, pendingSkill, target, durationMs });
  }
}
...
function pointCenterForHost(pointId, { boardSize, host }) {
