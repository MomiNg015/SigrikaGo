...
云端测试期间必须使用 `NODE_ENV=production`。此环境下，彩虹豆豆跳跳糖暂时不能对西格莉卡使用：库存接口返回 `disabledCharacterReasons.sigrika`，仓库将她标为“暂不可用”，服务端对直接使用请求返回 403，且不扣道具、不写效果或剧情进度。其他角色保持原规则。本地开发环境继续允许测试西格莉卡剧情；已有剧情进度和效果不清除，恢复与取消效果入口保留。剧情完成上线时，再移除 `server/items.js` 中的临时生产限制。无需新增环境变量或重新导出后台默认快照。
