...
- The home utility dock exposes a gacha entry that opens `GachaModal`. The player modal uses a Bright School compatible school-club capsule-machine layout: left-side ticket tabs show each open pool name, featured prize image stack, and date range; the main panel shows the featured prize group, remaining time, prize/history round buttons, coin/blue-gem wallet, and configurable single/ten draw buttons. Draws play a CSS capsule-machine motion before showing a nested result dialog. `AdminGachaPools` adds the admin "gacha" tab for pool price, schedule, featured prizes, and prize probability editing, with compact prize-probability summaries in the pool list. Prize rows select from the current admin-loaded resource catalogs rather than free-form item names or ids; choosing a resource copies its target id, display name, and image into the saved prize payload.
