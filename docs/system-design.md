...
- Board move preview and mobile touch-confirmation markers are point-local visual effects anchored with `left/top: 50%` and `translate(-50%, -50%)`, so ChangLi's extra-turn ordinary placement hints stay on board intersections instead of drifting into cell centers.
