...
- Character duplicate rewards increment `UserCharacter.chainCount`; public user payloads expose `characterChains`, and `CharacterChainBadge` overlays the chain count on portrait surfaces. Counts from 1 to 5 render as yellow five-point stars; counts above 5 collapse to `★×n` so compact portrait cards do not overflow. Player account mutation routes that return a refreshed user, including sortie-character and stone-decoration updates, must include the user asset relations used by `publicUser()` so member-manual rerenders do not replace existing chain counts with an empty map.
