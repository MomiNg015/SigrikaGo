...
- 玩家信息区的段位和积分分列为棋子上下两枚标签；积分显示追加“分”，技能超频使用红色强调。除子与超频标签支持桌面悬停说明和移动端点按说明：除子说明其会在数目时按 `+除子*1` 计入，超频说明其会在数目时按 `-超频*2` 扣减。移动端的除子、超频、技能说明从点击位置生成浮层，但定位会按浮层最大宽度和视口边距夹取，顶部空间不足时改为向下展开，并用最大高度和内部滚动保证内容不溢出画面外。
