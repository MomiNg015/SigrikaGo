...
- Game modes are now explicit shared runtime configuration. `spark` (`星炬对弈`) stays first and keeps the 13-line, skill-enabled rule set with black komi 2.75; `standard` (`标准对弈`) is second and uses a 19-line no-skill Go board with black komi 3.75. Frontend and backend should read titles, rule copy, board size, komi, skill enablement, time controls, and mode ordering from `src/shared/gameModes.js` instead of hard-coding mode facts.
...
- `server/roomSkillResolution.js`
  - 技能演出延迟结算的纯数据 helper。
  - 负责生成可持久化的 `pendingSkillResolution` 快照、计算恢复后的剩余结算延迟，以及判断快照是否具备重新调度条件；真实定时器、广播和回合推进仍由 `server/rooms.js` 负责。技能预览总时长来自 `src/shared/skillPresentation.js`，覆盖技能横幅展示和随后棋盘 PixiJS 演出。
...
- 首页匹配入口和好友约战入口共用对弈模式选择弹窗；模式按钮始终按星炬对弈、标准对弈排序，取消按钮必须与模式选项组保持明确垂直间距，桌面端和移动端都不能贴近最后一个模式按钮。
...
- `POST /api/me/music-selection`
  - 更新当前用户某个角色的技能 BGM 选择，body 为 `characterId` 和 `trackId`。
  - 后端只允许选择该角色可用且用户已拥有或默认解锁的 `skill` 类型曲目；成功后写入 `User.musicSelections.skill[characterId]` 并返回新的公开用户信息。
...
- `music` 类商品的 `targetId` 指向 `MUSIC_TRACKS` 中已有曲目；购买后写入 `User.ownedMusicIds`，默认解锁曲目和已拥有曲目不可重复购买。
...
- 实时房间仍以内存 `Map` 为核心状态，但单实例重启恢复已经补齐关键调度兜底：
  ...
  - 真实多进程或多实例部署仍不是当前架构目标；`npm run check:production` / `validateProductionDeployment()` 会拒绝显式多实例配置（如 `WEB_CONCURRENCY > 1`），避免误把内存房间架构部署成 cluster。若后续需要横向扩展，应引入共享房间状态、Socket.IO adapter、跨实例定时任务恢复策略和快照版本迁移。
...
- Playwright 浏览器回归基座已接入 `npm run test:e2e`，当前覆盖应用启动、关键 CSS 语义契约，以及真实 HTTP refresh-cookie 的注册/刷新/登出链路；后续应继续覆盖 Socket 重连、强制下线和房间恢复交界处。
...
- 数据模型仍有多处字符串化资产字段：
  ...
  - `server/userAssets.js` 已集中角色/装饰资产字段和道具库存字段的解析、去重和序列化，并提供 `legacyUserAssetsToStructuredRows()` 与 `syncStructuredUserAssets()` 将当前 CSV/JSON 字段投影/替换式双写为结构化迁移行。
  - Prisma schema 和 migration 已准备 `UserCharacter`、`UserDecoration`、`UserItem`、`UserItemEffect`、`UserProgressLedger` 表；商城购买、后台用户资产编辑、道具使用/消耗和对局结算效果清理已在更新 legacy 字段后同步结构化资产表。`publicUser()` 已支持在调用方加载结构化关系时合并结构化表与 legacy 字段，避免回填滞后或接口返回形态不同导致前端拥有状态跳变；认证、刷新、Socket 用户查询、排行榜、社交资料和后台用户列表/详情已显式加载结构化资产关系。历史账号可运行 `npm run backfill:user-assets`，由 `server/userAssetsBackfill.js` 分页读取 legacy 字段并调用 `syncStructuredUserAssets()` 回填结构化表。后续新增用户展示入口应复用 `publicUser()`、`USER_ASSET_RELATION_INCLUDE` 或 `USER_ASSET_RELATION_SELECT`，避免重新直接解析 legacy 字符串。
...
- 音频播放逻辑已拆出 `audioSettings.js`、`backgroundDucking.js`、`backgroundMusic.jsx` 和 `effectPlayback.js`；`playback.jsx` 保留兼容 re-export，并继续承载技能语音、系统语音、语音缓存和 TTS。后续可继续拆 voice、audio cache 两类模块，并补浏览器自动播放策略下的集成测试。

- 技能扩展已把具体主动技能 handler 下沉到 `src/shared/gameSkillHandlers.js`，但 `paramsJson` 尚未形成完整通用规则契约；`random-blast` 的完整 3x3、演出延迟、回放重放、棋盘高亮清理等逻辑仍分散在共享规则、服务端房间和前端棋盘中。后续新增角色时应继续抽技能契约，避免重新堆分支。
...
- `src/audio/effectPlayback.js`
  ...
  - `src/app/InteractionFeedback.jsx` installs document-level interaction feedback: enabled button-like controls play the confirm SFX, disabled/locked controls play the unavailable SFX, and the unavailable target receives a `ui-unavailable-shake` animation whose duration matches `ui_unavailable.ogg`. Specialized interactions can set `data-ui-sound="none"` to avoid double-playing the generic confirm cue. Room board interactions, player-info panels, the room title/info stack, and replay control bars are quiet zones for generic confirm SFX so normal moves, high-frequency info taps, and replay stepping do not fire UI prompt sounds; replay disabled buttons can still fire the unavailable SFX.
...
- 角色详情弹窗右上角包含独立的角色 BGM 预览播放器。播放器显示当前曲名，支持播放/暂停并保留 intro/loop 进度；当该用户拥有同一角色的多首技能 BGM 时显示下拉选择，切换后通过 `/api/me/music-selection` 持久化并同步影响后续对局 BGM。
- 角色 BGM 预览播放期间会向全局 `BackgroundMusic` 发出暂停请求，暂停大厅/对局背景 BGM 的 Web Audio context；预览暂停、切歌或关闭详情弹窗时释放该请求并恢复原 BGM 进度，避免双 BGM 叠播或大厅 BGM 从头重启。
...
- 星炬棋盘技能特效的程序化音效由 `src/audio/skillEffectSounds.js` 负责，按 `effectType` 在棋盘演出的开始和命中关键帧播放；这些音效只读 `audioSettings.sfx`，不参与服务端结算，也不会在 reduced-motion 棋盘演出中播放爆炸/飞入类音效。
...
- The game-start system voice is only played for active player views in `playing` phase. Spectators and finished-room viewers do not replay historical `game-start` messages when joining through the watch list. Socket reconnects mark the next live player room snapshot with `__audioResumeBaseline`, and duplicate reconnect snapshots with the same room/history/chat key keep that marker so the server's immediate `room:update` plus `broadcastRoom` pair cannot overwrite the baseline before `RoomScreen` mounts and accidentally replay old `game-start` chat.
...
- The preload step fetches non-replay runtime assets after login: all current character portraits, home and shop imagery including `/assets/home/multipurpose-classroom-bg.webp`, Denia candy GIF portrait, stone decoration images, the Denia bubble-effect preview, common board/result/match/UI interaction sound effects, every configured BGM track, every configured character skill voice, and every configured character system voice. Replay lists and replay details remain lazy data requests so opening the app does not prefetch historical game records.
