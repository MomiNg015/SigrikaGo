...
- The home utility dock exposes a gacha entry that opens `GachaModal`. The player modal uses the selected Scheme C school-festival candy-capsule layout, scoped by `.gacha-festival-scheme` so it stays separate from admin gacha management: desktop uses left-side festival ticket tabs for each open pool name, featured prize image stack, and date range, then a central candy capsule machine stage with a festival marquee, featured prize group, remaining time, prize/history round buttons, coin/blue-gem wallet, and configurable single/ten draw buttons. Mobile switches the same modal to one column with horizontally scrollable pool tickets, a compact marquee/machine stage, and a sticky bottom control panel so wallet and draw buttons remain reachable. Draws play a CSS capsule-machine motion before showing a nested result dialog; ten-pull results use a desktop 2-by-5 reward grid, each reward card shows the prize image plus player-facing display name/details, and coin rewards use the local `/assets/items/gacha-coin-bag.svg` image. `AdminGachaPools` adds the admin "gacha" tab for pool price, schedule, featured prizes, and prize probability editing, with compact prize-probability summaries in the pool list. Prize rows select from the current admin-loaded resource catalogs rather than free-form item names or ids; choosing a resource copies its target id, display name, and image into the saved prize payload.
