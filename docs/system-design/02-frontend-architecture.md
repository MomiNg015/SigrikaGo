...
- Player-facing `.modal-backdrop` and `.nested-modal-backdrop` surfaces share the CSS-only paper-landing entrance from `src/styles/modals/window-motion.css`. The animation targets immediate semantic window children, preserves component ownership and synchronous focus, and composes with existing centering transforms through individual translate/scale/rotate properties. It must not add perspective/transform to backdrops or retain transform longhands after entry, because either can make fixed nested replay/profile/detail windows relative to their parent modal. Character opening remains an explicit cinematic exception; result, match-success, story, and tutorial sessions use the calmer shared variant.
