...
- `appShellProps` / `overlayPropsFromState`: 位于 `src/app/appShellProps.js` 和 `src/app/overlayRegistry.js`，把 `App.jsx` 到 `AppRoutes` / `AppOverlays` 的 route state、route actions、overlay state/actions 和 registry-derived `show*` / `setShow*` props 收口成 `routeProps` 与 `appOverlayProps`。新增路由或应用级弹窗时，应先扩展 registry/适配层测试，再让 App 根组合展开适配对象，避免重新堆出手写 JSX prop 清单。
