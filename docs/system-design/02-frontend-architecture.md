...
- Bright School board guards keep `.board-row-effects` transparent and overflow-visible, and keep `.board-row-slash` plus its `::before`/`::after` highlights as the only painted slash elements. This protects the DOM-only `row-slash` overlay from inherited paper-surface cleanup, which otherwise can cover the grid and stones with a blank panel or flatten the slash into a plain bar.
