...
- Player-facing `.modal-backdrop` and `.nested-modal-backdrop` surfaces share the CSS-only center-reveal entrance from `src/styles/modals/window-motion.css`. The animation targets immediate semantic window children and opens the image symmetrically from a horizontal center line through temporary clip-path and opacity stages. It preserves component ownership, synchronous focus, existing centering transforms, and viewport-relative fixed nested replay/profile/detail windows because it never adds transform or perspective and does not retain animation fill. Character opening remains an explicit cinematic exception; result, match-success, story, and tutorial sessions use the shorter shared variant.
