...
- 后台管理新增桌面端分析体验：`AdminOverview` 是默认“今日简报”，用可读总状态、原因、分级解读和下一步动作替代密集表格；`AdminOperations` 是运营分析页，先显示推荐解读，再用低密度 CSS 条形图和卡片展示活跃、注册、对局、分层、经济和模式表现。分析样式集中在 `src/styles/admin/analytics.css`，最终后台控件皮肤集中在 `src/styles/admin/polish.css`，用于统一浅色按钮、输入框、表格、tabs、危险操作和关闭按钮；该 polish 层的 `polish/hud-isolation.css` 子文件负责在 `.admin-screen` 内压过 Startorch/HUD 的全局 `.primary-action`、`.danger-action`、`.close-button`、`.modal-backdrop`、`.confirm-modal`、普通 `input`/`textarea`/`select` 和 `div[class*="setting"]` 终端皮肤，确保后台桌面端和移动端不继承黑底、绿纹、斜角按钮、黑色输入框或霓虹阴影。
...
- `BoardSkillEffects` 的技能演出合同是不降级的真实 Pixi canvas：严格 CSP 下 `src/room/pixiPrewarm.js` 先加载 `pixi.js/unsafe-eval` 再加载 `pixi.js`，`preparePixiEffect()` 在技能横幅窗口内并行启动 Pixi app 初始化与阻塞型 Pixi sprite asset preload，只有两者都完成后才把 `.board-effects-canvas` 挂到 overlay host 并让 `playRegisteredBoardSkillEffect()` 进入可见播放。这个边界确保 PixiJS v8 的 `Sprite.from()` 只读缓存时不会在贴图未 decode 时读到空纹理或白帧；结算后 DOM marker 资源仍可走非阻塞预热，不能延迟不依赖贴图的程序化 Pixi cast。协议接管和陨石擦除等临时 Pixi 残留层需要在演出结束前淡出，收尾清理必须先把 `.board-effects-canvas` 从 overlay host 摘除，再销毁 Pixi/WebGL app，避免 WebGL context teardown 在仍参与合成时露出底帧。`playRegisteredBoardSkillEffect()`、asset preload、ticker callback 或 renderer 输入校验失败时只标记 `data-effect-failed` / `data-effect-error` 并清理 overlay，保持 React 房间不白屏；没有 DOM/CSS 替代动画，准生产稳定性测试必须看到真实 `.board-effects-canvas` 才算技能演出通过。Vite 开发期在 `vite.config.js` 中将 `pixi.js` 与 `pixi.js/unsafe-eval` 排除出 dep optimizer，并显式 include `pixi.js > @xmldom/xmldom`、`pixi.js > eventemitter3`、`pixi.js > gifuct-js` 和 `pixi.js > ismobilejs`，避免 Pixi 懒加载 WebGLRenderer 等 renderer 模块时命中浏览器缓存里的过期 `.vite/deps` chunk，同时保留 Pixi 源码路径里 CommonJS/conditional-export 子依赖的 ESM 互操作。
...
- `modalDismissal`: lives in `src/app/modalDismissal.js` and owns the shared topmost-modal dismissal contract. Desktop Escape and browser/mobile history back close only the current top modal; app overlays, result/match-waiting modals, and the home match-mode picker should use this shared mechanism instead of local keydown/popstate listeners. When no modal is active, the mobile root-back guard intercepts phone/browser back on login, preload, home, and admin screens and shows the shared confirm modal with “确定要退出游戏吗？” before allowing the browser to leave the app. A loaded room instead forwards a monotonic back-request id through `AppRoutes` to `RoomScreen`, which invokes the same `requestExitConfirm` callback as the visible leave-room control: replay and spectator views return directly home, while an unfinished player game asks whether to resign and exit.
...
- Admin character drafts keep code-defined derived skills in the base skill `params.derivedSkills` JSON through `buildCharacterDraft` and `characterDraftToBody`, but the editor is content-only: both the base skill and every existing derived skill expose only name, description, and overclock. Draft construction never invents a missing definition, serialization preserves each definition's identity and logic fields, and there are no add/remove or effect-rule controls. Future characters become editable automatically after their derived definitions are added to code/default data.
...
- `ShopModal.jsx` owns the semantic 扎希拉商铺 shell and refresh/title/close header. `useShopCatalog.js` owns the open-session catalog, eligible-item filtering, five-item current/prepared batches, three-second refresh cooldown, targeted next-batch preload, purchase reconciliation, and contextual mascot dialogue. `ShopItemCard.jsx` derives category and optional remaining-quantity corner badges from shared helpers without duplicating stock semantics; the card shell contains a native detail trigger for image/name/price and a sibling purchase button, so keyboard detail access no longer depends on a pseudo-button article containing another button. `ShopProductStage.jsx` plus `shopLayout.js` own the shared count-aware 2+3 / 2+2 / 2+1 topology: desktop keeps seeded position jitter inside balanced collision-safe row cells, while mobile uses the same centered rows with whole-card scaling and no internal scroll; final card-layout owners ensure the algorithmic slot and rendered card retain the same base dimensions through the theme cascade. The existing server purchase endpoint remains authoritative.
...
- Main-time timer digits use dark `#1c171a`, while byo-yomi states keep their warning colors.
...
- Vite dev dependency optimization intentionally excludes Pixi runtime entries (`pixi.js` and `pixi.js/unsafe-eval`) because Pixi selects WebGL/WebGPU/Canvas renderer modules through lazy dynamic imports, and optimized renderer chunk filenames can drift across dev-server and browser-cache state. It still explicitly includes Pixi nested runtime dependencies (`pixi.js > @xmldom/xmldom`, `pixi.js > eventemitter3`, `pixi.js > gifuct-js`, and `pixi.js > ismobilejs`) so Pixi source modules do not load raw CommonJS entries without the default or named exports they import.
...
- Bright School board guards keep `.board-row-effects` transparent and overflow-visible, and keep `.board-row-slash` plus its `::before`/`::after` highlights as the only painted slash elements. The row-slash owner rule must also keep `.board-row-slash` at `max-width: none !important`; otherwise the portrait mobile all-element `max-width: 100% !important` safety layer caps the absolutely positioned scar to the parent board width while its negative left offset survives, making the right side short. This protects the DOM-only `row-slash` overlay from inherited paper-surface cleanup, which otherwise can cover the grid and stones with a blank panel, flatten the slash into a plain bar, or shorten the scar on mobile.
...
- `src/admin/storyScriptWorkbook.js` owns browser-local `.xlsx` conversion for admin剧情教学 scripts. `AdminOnboardingStory` only orchestrates file selection, card-level export, toolbar import, validation feedback, and the confirm-before-apply change summary; workbook parsing never uploads files, never reads `发布版-*` or `原始JSON` sheets for import, and applies a valid import only to the current draft/dirty editor state.
