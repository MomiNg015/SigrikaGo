...
- 彩虹豆豆跳跳糖对爱弥斯使用时同样先判定 35% 拒绝概率；拒绝分支不消耗道具、不应用效果。接受分支消耗 1 个道具并将 `itemEffects.aemeathRainbowMove` 置为 true，棋舍以“彩虹落子模式”徽标展示；对局展示层只在当前最后一步确实由带有效果的爱弥斯落子时，于对应交叉点播放约 620ms 的七彩像素光纹，棋子仍保持普通黑白外观且不改变规则、技能或数值。光纹标记指针穿透，连续关键帧仅改变 `transform` 与 `opacity`，`prefers-reduced-motion` 下冻结为局部静态彩环；爱弥斯完成一盘有效对局后清除该状态，使用其它角色完成对局不会误清除。
