...
- `src/admin/AdminConsole.jsx`
  ...
  - 具体 tab 已拆到 `src/admin/AdminOverview.jsx`、`AdminOperations.jsx`、`AdminUsers.jsx`、`AdminCharacters.jsx`、`AdminShopItems.jsx`、`AdminDecorations.jsx`、`AdminMascotSettings.jsx`、`AdminSiteSettings.jsx`、`AdminFeedback.jsx`、`AdminAudit.jsx`；其中“看板娘管理”统一维护扎希拉、娜波摩和 IRIS 台词，并在 IRIS 分区继续维护友情链接，不与系统设置共用编辑草稿。共享后台展示零件和 helper 位于 `adminComponents.jsx`、`adminFormatters.js`、`adminUserDrafts.js`。
...
- 大厅首页布局 A：星炬对弈作为最大主行动面板；该区域使用 `/assets/home/fantasy-match-illustration.webp` 透明 PNG 插图作为主体，插图顶部与左侧部员手册区域顶部对齐，不显示原先标题、说明文字、边框或卡片背景，只在插图下方保留较小的“开始匹配”按钮。当前用户铭牌作为履历入口展示出战角色、用户名，以及按星炬、标准、五子棋三列排列的模式图标和段位数字，不直接显示积分；部员手册作为角色/装饰入口；商店、排行榜、观战、好友以中等图标按钮呈现。顶部条显示来自站点配置的标题和版本号，版本号中的拉丁/数字走 WuWa 艺术字体；在线人数标签退出 Header，品牌与操作区之间改为响应式装饰看板娘，右侧保留操作按钮；后台管理仅管理员可见，桌面位于操作区，移动端收纳进折叠菜单。
- 大厅主内容区尽量靠近窗口垂直中部；履历铭牌、部员手册和工具按钮位于左侧，星炬对弈主面板位于右侧。星炬对弈上方显示横向用户铭牌：出战角色头像、用户名，以及按模式排列的图标和段位数字，铭牌不直接显示积分；大厅在线人数仍可用于匹配统计数据，但首页 Header 不再渲染其独立 HUD 标签。
...
- `OpeningDuelPresentation` 在猜先阶段以两条全高窄幅斜切条、自己右侧、对手左侧的立绘替代普通弹窗外框；桌面每条宽度为 min(26vw, 360px)，手机每条 42%，背景棋盘和信息区仍可见并由独立 62% 黑色遮罩统一压暗，遮罩随退场淡出恢复明度；双方相向上下入场约 500ms 并轻微回弹，中央直接显示执棋结果、规则和倒计时。手机竖屏对手偏左上、自己偏右下；条带底色按棋色，立绘下用户名使用黑方白字／白方黑字，单行测量缩字并保留斜切安全区以完整显示。`socketHandlers` 仅对当前 `match:found` 玩家会话添加客户端 `__openingPresentation` 标记，后续快照保留，重连或 resume 撤销；旁观和中途加入不补播。立绘复用 `playerCharacterForDisplay` / `playerCandyPortraitProps` 的外观与时装快照，任一方既无角色也无机器人专用立绘、图片失败、普通对局 200ms 内未就绪、吃子挑战赛预热后最多等待 1500ms 仍未就绪，或剩余时间不足 900ms 时回退原提示。演出 portal 到 body，使用独立黑白样式避免房间裁切和主题模态框覆盖；保留减少动态效果的淡出替代。左右拉幕持续 400ms，并在 `openingEndsAt` 前完成；到点移除覆盖层，离开 opening 也立即卸载，不延长服务端倒计时或添加音效。
...
- 对手信息区下方显示“房间成员”，固定最多 3 行高度并可滚动；回放模式不显示房间成员区域。对局者用黑/白棋图标标识执色且用户名为红色，观战者为黑色用户名。点击任一行会展开操作面板：详细信息、加好友/解除好友、加入黑名单/从黑名单解除；未实现的“密谈”不再展示，三个保留按钮在同一行内等宽分布。自己所在行的加好友和黑名单操作禁用；好友关系下加入黑名单按钮禁用。成员行按关系显示背景：自己为淡黄色，好友为浅绿色渐变，黑名单成员为灰色。加好友会自动从黑名单移除目标用户。
...
- 技能类型：
 ...
 星星落地后显露打瞌睡的日灵；结算后的无效点继续使用 `/assets/effects/sigrika-erased-field-marker.webp` 作为 point-local 透明 WebP 日灵标记，以 150% 棋盘格尺寸显示。棋盘的 `erased-boundary-layer` 灰显周围格子、用一路线同粗细外边界线标出边界；演出层从星星接触时开始用 200ms 交叉淡化显现该区域，结算后交回权威边界，不提前改变点位规则。
...
- 技能目标确认态由前端 `pendingSkill` 驱动：`ActionBar` 给技能按钮添加 `.active`，`Board` 给棋盘容器添加 `.board-wrap.targeting` 并给可用目标点添加 `.previewable`。这个值属于客户端房间内草稿，`useRoomSessionState` 用 `room.code + room.role` 记录其所有者；同房间结构共享或普通全量快照不会清空草稿，但不同房间在第一次渲染时就读为 `false`，不依赖渲染后的 effect。`RoomScreen` 的普通认输和“认输并退出”确认都会先清除草稿，再发送 `resign` 并按需返回大厅。Bright School 的最后 CSS 层必须保留技能按钮与棋盘外圈的彩色动画发光，并让星位点使用独立 `::after` 居中显示；旧的星位 `::before` 必须显式关闭，避免和目标预览光圈使用的 `::before` 冲突并产生虚假星位。
...
- 玩家商城大厅入口显示为“商店”。商店窗口分为左右两列：左侧窄列为扎希拉接待区；桌面端金币钱包固定在接待区从上到下约 30% 的位置，问候气泡与底部等宽立绘之间保留约 5% 信息区高度间距，默认立绘 `/assets/zahira_shop_default.webp` 等宽贴在接待区底部，商店看板娘 WebP 由保留的 PNG 源图无损编码并保持 1448x1054 源尺寸。移动端接待区位于商店上方，金币钱包保持左下区域，问候气泡保持左侧主列，立绘位于右下并占接待区宽度的 50%，所有立绘只按比例缩放。购买成功后看板娘切换到 `/assets/zahira_shop_laugh.webp`，问候语切换为“谢谢惠顾！”；该反馈不再通过五秒计时器自动恢复，残星会看板娘娜波摩的购买成功/失败反馈也遵循同一持久规则，只由刷新、重试或关闭商店等显式动作改变。`ShopSidebar` 同时挂载默认和感谢两张立绘，并用透明度图层切换避免购买反馈时出现白色闪烁。右侧宽列直接从分类选项卡进入商品栏和页码，不再显示大标题。商品区每页仍固定 8 个槽位，但视觉网格会按可用宽高自适应列数，商品区内部独立滚动，页码固定保留在底部且当前页使用浅色高亮；商品网格内容在选项卡和页码之间使用安全居中：高度足够时尽量让第一行商品顶部与选项卡底部、第二行商品底部与页码按钮之间的留白接近，高度不足时回退到顶部对齐，保证滚动条可以覆盖完整商品栏。横屏小高度或窄窗口下会压缩扎希拉立绘、气泡、商品卡片和选项卡间距，避免商品栏和分类选项卡被挤出视口。商品数超过 8 个时自动增加页码，点击页码切换对应商品栏，切换分类会回到第 1 页且不改变槽位数量。空槽显示“暂未上架”，可购买槽使用简明浅色渐变，已拥有槽使用浅绿色渐变。
...
- 对角色使用道具成功后，页面顶部 toast 显示“对[角色名]成功使用了[道具名]”；角色拒绝彩虹豆豆跳跳糖时改为显示“[角色名]拒绝了[道具名]，道具未消耗”，并使用非成功提示。使用结果窗口读取接口返回的最新 `user.itemEffects`，所以达妮娅接受并吃下彩虹豆豆跳跳糖后，结果窗口中的达妮娅立绘会立即显示为彩色 GIF 状态。
...
- 内置道具“彩虹豆豆跳跳糖”会 seed 为 `item` 商品，商店库存 10、价格 10、图片 `/assets/items/rainbow-bean-candy.webp`，介绍为“产地不明的糖果，据说有神秘的效果”。服务端会把商品、仓库和玩家侧抽卡奖项/结果中的内置糖果旧 PNG 图路径归一为当前 WebP，默认快照里的糖果商品和卡池奖励也使用同一路径。该道具只能对角色使用，目前西格莉卡、达妮娅、爱弥斯和琳奈有实际效果；`src/shared/rainbowBeanCandy.js` 统一登记支持角色及效果键，仓库目标选择不再维护独立白名单。其它角色会返回“这个角色暂时没有糖果效果”且不消耗道具。
- 彩虹豆豆跳跳糖对西格莉卡使用时先判定 35% 拒绝概率；拒绝分支不消耗道具、不改变金币、效果或出战角色。接受分支消耗 1 个道具且不再奖励 30 金币，`itemEffects.sigrikaCandyDisabled` 置为 true；西格莉卡棋舍出战按钮变灰，`/api/me/character` 与 socket 选角都会避开西格莉卡。如果当前出战是西格莉卡，后端自动切换到一名已拥有且可出战的其它角色；没有可替换角色时在随机判定前拒绝使用且不消耗道具。成功剧情明确她去找医生且暂时不能找她下棋；任意角色完成一盘有效对局后清除该状态。
- 彩虹豆豆跳跳糖对达妮娅使用时同样先判定 35% 拒绝概率；拒绝分支不消耗道具、不应用效果且不触发“你给我吃了什么！？”成就。接受分支消耗 1 个道具并将 `itemEffects.deniaRainbowGlow` 置为 true，剧情和特殊立绘表现为她的双眼与嘴巴发出三道彩虹射线，而不是全身发光；除用户铭牌外，所有展示该用户达妮娅立绘的位置都会把原立绘替换为 `/assets/characters/portraits/denia-candy.webp` 彩色动图，包括棋舍、仓库使用结果、对局信息区、技能横幅、排行榜/好友资料等可获得用户效果状态的界面。该规范资源保留 16 帧、每帧 70ms 和无限循环元数据，并与静态角色立绘共用 900×900 单帧画布、792px 安全框和底部中心锚点。只有使用达妮娅完成一盘有效对局后清除该状态；服务端会在首个有效结算 `room:update` 广播前先更新房间内存玩家状态，前端再从房间视图同步当前用户状态，因此返回大厅/棋舍时立绘会恢复为角色原始立绘。
- 彩虹豆豆跳跳糖对爱弥斯使用时同样先判定 35% 拒绝概率；拒绝分支不消耗道具、不应用效果。接受分支消耗 1 个道具并将 `itemEffects.aemeathRainbowMove` 置为 true，棋舍以“彩虹落子模式”徽标展示；对局展示层只在当前最后一步确实由带有效果的爱弥斯落子时，于对应交叉点播放约 520ms 的电子彩虹反馈：光源使用棋子抖动后的实际偏移，保证与棋子视觉中心重合；四条七色细光贴合棋盘横纵线延伸至对应边缘，整体透明度由中心向外平滑衰减，沿途交叉点按距离依次点亮且逐渐变淡，七个频率残影在中心附近错位弹开，不再使用圆形渐变环。明亮学园竖屏全局宽度限制由该特效 owner 定点解除，防止长光轨被裁成单格。棋子仍保持普通黑白外观且不改变规则、技能或数值。标记及所有子层均指针穿透，连续关键帧只改变 `transform` 与 `opacity`，`prefers-reduced-motion` 下冻结为紧凑的静态网格光标；爱弥斯完成一盘有效对局后清除该状态，使用其它角色完成对局不会误清除。
- 彩虹豆豆跳跳糖对琳奈使用时同样先判定 35% 拒绝概率；接受分支消耗 1 个道具并将 `itemEffects.lynaeContraryVoice` 置为 true，棋舍以“反话语音模式”徽标展示。效果只重映射琳奈的系统语音事件：读秒 10 到 1 反向播放为 1 到 10，剩余读秒次数 2↔1、开局↔开始读秒、出战↔技能、胜利↔失败；和棋不交换，界面文字、计时状态、技能行为与胜负结算均保持真实值。琳奈完成一盘有效对局后持久效果清除；服务端在清除前把效果复制到房间玩家的 `completedItemEffects`，只供当前结果弹窗播放错位的胜负语音，不能继续影响下一局。
- 西格莉卡、达妮娅、爱弥斯与琳奈的默认糖果脚本都在一份已发布记录内包含 `accepted-start` / `rejected-start` 两套节点，服务端按本次 `itemUseOutcome` 选择响应起点。叙述节点保持 `speakerName: ""` 与 `characterId: ""`，剧情窗口角色区域为空，不渲染“旁白”字样。
- 登录后的静态资源预加载会无条件加入达妮娅糖果动图 `/assets/characters/portraits/denia-candy.webp`，让用户之后在棋舍、排行榜、好友资料或对局中首次看到彩色达妮娅时不再临时下载；对局回放数据仍保持按需加载。
- 糖果效果只在有效对局结束并保存棋谱的链路清除；不超过 10 手的无效对局不会清除糖果效果。达妮娅、爱弥斯与琳奈的角色专属效果仅在使用对应角色完成有效对局时清除，使用其它角色不会误清除。
...
- 自己的履历与他人详细资料共用 `ProfileResumeView`：模式资料仍读取当前模式持久化 `rating` 与棋谱派生的胜负和数据，页签渲染层显式锁定为“星炬、标准、五子棋”，胜率由胜局/总局派生。近全屏桌面外壳内使用上限 1160px 的居中档案主体，共享身份页眉、段位/积分/总对局/胜率摘要、最近十盘和语义化角色成绩表；主体结构层保持透明以连续露出外层档案纸纹。自己的身份区只提供“个性化”，页眉右侧依次保留“成就、金币、关闭”，他人资料身份区提供“点赞、加好友、举报”，而“加入黑名单”留在底部低优先级关系区并先二次确认。桌面资料体以显式命名网格区固定摘要、角色滚动区与底部操作，长角色列表不会再把黑名单推到窗口外；资料标题与关闭按钮共享对齐轨道。两类入口的对局回放统一复用 portaled `HouseReplayDialog`，作为 `.app-shell` 下的独立顶层弹窗打开，关闭后保留下层档案并恢复焦点。模式切换在成功前保持旧页签与旧资料对齐，显示载入/重试状态、锁定重复请求并忽略过期响应；举报失败保留输入并在嵌套弹窗内展示。所有弹窗继续使用 `ModalDialog` 的 Escape、焦点循环和关闭后焦点恢复合同；接口和数据库结构不变。
