...
早期生产库曾出现 `_prisma_migrations` 已记录当前基线、实际 SQLite 却缺少后来进入模型的列（例如 `Character.illustName`）的历史漂移。维护脚本在停服、备份和 `prisma migrate deploy` 之后运行 `npm run production:schema-compat`，复用 `SERVER_SCHEMA_TASK_ORDER` 中已有的幂等 schema guard 补齐这类已知兼容结构，再执行后台默认快照同步。该过渡兼容层不登记或伪造 migration，也不能成为继续修改 `0_init` 的理由；今后的 schema 变化仍必须从冻结基线新增并提交 migration。

### Admin Deployment Defaults

Non-user admin deployment configuration lives in `server/adminDefaultSnapshot.js`, generated from local `prisma/dev.db` with `npm run admin:snapshot`. The exporter includes admin-owned `SiteSetting` rows (including recruitment configuration), skill traits, characters/skills (including CV and default-costume illust credits), decorations, costumes (including display names, descriptions and illust credits), shop items, gacha pools/prizes, achievement rewards/achievements, music display names, story/tutorial scripts, announcement/changelog entries, and the legacy onboarding singleton, while filtering internal `migration.*` markers and excluding every user-owned/history/runtime domain. `npm run check:admin-snapshot` structurally compares these collections with the ignored local database and is part of `npm run check`, so a local admin save cannot silently miss the next commit. Ordinary server startup remains create-only through `seedAdminDefaultConfig()` and therefore never overwrites a cloud admin edit merely because the service restarted. Formal deployment deliberately uses `npm run admin:sync-defaults` after backup, migrations and schema compatibility: preview reports create/update/unchanged/cloud-only counts, and `-- --apply` updates rows represented by the committed snapshot and creates missing rows without deleting cloud-only rows or touching users, owned assets, purchases, draw history, feedback, reports, audit logs, analytics, announcement reads, mailbox data, game records, or live rooms. The scoped onboarding export/sync commands remain for exceptional single-story maintenance, but the maintained production update script uses the full non-user sync contract.
...
- The Aemeath welcome message is a direct system message with `batchId = null`; registration creates it in the same transaction as its recipient. Its fixed attachment id is also seeded as a disabled `ShopItem` so Warehouse/admin attachment metadata remains available without exposing it in the player shop. Mailbox list projection resolves item attachment display metadata from this catalog row and falls back to the shared built-in recruitment item definition; the persisted message continues storing only stable id/type/quantity claim data.
...
- 西格莉卡的默认糖果脚本在同一发布图内预留 `use-1-start` 至 `use-7-start`、共享效果尾段、`corruption-start`/高潮，以及胜负两条恢复起点。当前仅交付可编辑结构和占位文案；`seedDefaultStoryScripts()` 只把节点签名完全匹配旧内置默认值的脚本升级为新框架，管理员已定制或已是新结构的脚本不会被启动种子覆盖。
...
- `User.onboardingRequired` defaults to `false` for historical users and is set to `true` when a new user registers after the feature is available. `User.onboardingAutoShownAt` is null until the automatic story/tutorial modal is opened successfully from the home view. Recording auto-shown sets `onboardingRequired = false` and writes `onboardingAutoShownAt` once; `User.onboardingCompletedAt` is written only by the explicit completion endpoint. `User.welcomeMailNoticeShownAt` independently records the first onboarding exit that found the system Aemeath mail, so closing, completion, and skipping share one toast gate without changing completion semantics. Browser refresh or later manual replay do not reopen the automatic story once auto-shown has been recorded.
- The generic schema is backed by migration `202606280003_add_story_scripts`, the tutorial initial-board migration `202606290001_add_story_tutorial_initial_board`, and startup guard `ensureStoryScriptSchema()`. The onboarding touch/completion fields and legacy table are backed by `202606280002_add_onboarding_story`, `202606290002_add_onboarding_completed_at`, `202607170002_aemeath_ticket_acquisition`, and `ensureOnboardingStorySchema()`.
- 西格莉卡篇章字段已经折叠进预发布单基线 migration `0_init`，并由 `ensureSigrikaCandyArcSchema()` 为既有本地数据库补齐；启动顺序先保证用户列可读，再 seed/处理对应剧情和玩家请求。
