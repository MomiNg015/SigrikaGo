...
- In production, `deploy/nginx/sigrikago.conf` is the primary traffic boundary: only `/socket.io/`, `/api/`, and `/health/*` reach Node; uploads and built assets are served by Nginx. Hashed Vite outputs receive one-year immutable caching, named `/assets/**` resources receive a one-hour cache plus 24-hour stale-while-revalidate, and `index.html` is no-cache. `server/staticAssets.js` retains the same cache contract as a Node fallback and for `LOCAL_PROD_STATIC=1` verification, with Express index serving disabled so the SPA fallback can set the HTML header explicitly.
