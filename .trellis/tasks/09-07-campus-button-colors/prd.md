...
## Follow-up: interior shadow gutters and square decoration cards
- User reported clipped button shadows and transient scrollbars in small windows, and requested icon-only square decoration cards with more padding and diagonal black/white stone placement.
- Confirmed the decoration scroller had no right/bottom shadow space and retained the global thin scrollbar; narrow settings content had zero padding. A final scoped owner now hides player-modal scrollbar chrome without resetting native overflow, and adds narrow content gutters to settings, friends, warehouse, ranking, watch and handbook lists.
- Decoration cards are 80px squares with 10px padding. Removed visible status/checkmark markup and its phone pseudo-element; retained accessible names, pressed/disabled state and added native busy semantics. The black and white preview stones stay scoped to handbook cards.
- Existing palette-hook findings are unchanged legacy colors and test fixtures. No suppressions or new literal colors were added.
- Short-phone visual QA exposed an additional existing four-row grid contract despite only three normal-flow children. The portrait shell now uses `auto minmax(0, 1fr) auto`, allowing the roster to scroll and keeping a complete decoration row visible.
- Browser QA passed across handbook, settings, friends, warehouse and confirmation windows at 320x568, 390x844 and 800x600. The final content buttons have hard-shadow gutters after scrolling to the bottom, stay inside every clipped ancestor, retain hidden scrollbar chrome while pressed, and cause no document horizontal overflow. Decoration geometry, padding, empty visible text and black/white diagonal positions are checked directly on the real components with synthetic data.
- Final validation: all 352 test files / 2506 tests, lint, portrait/snapshot checks, production build, built CSS and production configuration passed. The check command's final HTML write hit the existing transient Windows `UNKNOWN` error; a separate `npm run docs:system-design` rerun succeeded.
