...
- 当前加载条已有橙色吉祥物随真实进度移动、轻微进度时待机浮动、进度条 ARIA 语义与 reduced-motion 降级。
...
- 视觉增强必须复用现有 Bright School 色彩和橙色吉祥物，不引入通用科技感、散落纸屑或与主题割裂的视觉语言。
- 加载条采用明亮橘色果汁方向：内部颗粒与高光持续流动，小橘子保持正向并以轻微倾斜滑行，停顿时柔和浮动；外部只保留与真实填充同步的无边界暖光，不使用彩虹胶囊光框。

## Acceptance Criteria

- [x] 点击“登记入部信息”进入真实异步提交态后，两个 `.auth-password-toggle` 的垂直居中 transform 不变。
- [x] 密码可见性按钮的默认、hover、focus-visible、disabled 状态均无布局位移。
- [x] 加载条在 0%、中间进度、100% 下填充与吉祥物位置正确，轨道无水平溢出。
- [x] 进度移动与停顿时呈现不同但连续的状态反馈，且不会遮挡加载文案。
- [x] reduced-motion 下移除持续位移/旋转等非必要动画，同时保留清晰静态进度。
- [x] 相关 AuthScreen 与 AssetPreloadScreen 测试通过。
- [x] 桌面与竖屏移动端完成视觉检查。
...
**Decision**: 保留现有组件、资产和进度数据流，把视觉焦点收敛到果汁感动态填充和保持正向的小橘子。填充内部使用颗粒与扫光表达工作进行中；端点通过指数缓出平滑跟随进度，外部暖光跟随填充长度并以模糊消除硬边界。
