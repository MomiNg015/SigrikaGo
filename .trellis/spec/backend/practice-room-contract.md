...
const engine = createPracticeBotEngine({
  enginePath: process.env.PRACTICE_ENGINE_PATH || "/usr/games/gnugo"
});
...
- Zhunshibao's product-facing rank is difficulty-configured as `入门陪练`, `中级陪练`, or `高级陪练`. The legacy `basic` restore alias uses the beginner engine level and rank but remains a distinct persisted id. Keep rank, engine level, and capture threshold as separate fields.
- Every public tier uses the server-local GNU Go executable through GTP: beginner level 1 with a 2-second timeout, intermediate level 5 with 3.5 seconds, and advanced level 10 with 5 seconds. Each process uses an 8 MB cache and `--never-resign`. Do not retain a custom heuristic, tactical, or UCT move selector as a fallback.
...
- Before invoking GNU Go, enumerate the current bot-view moves with the real `playMove(..., { colorIllusion: null })`. Convert only successful point ids to GTP vertices and pass that list to GNU Go's `restricted_genmove`. This whitelist is the compatibility boundary for invalid points, protocol bans, liberty-purge bans, ko, suicide, occupied points, and other Spark-only legality. Convert the response back to a point id and revalidate it before `handleGameAction`.
...
- `ensureAvailable()` probes the configured executable with `--version` and caches success; failures are cached briefly to avoid a spawn storm. `practice:start` performs this check after authentication/active-room validation but before leaving matchmaking or creating a room. Unavailable engines return `practice_engine_unavailable`.
- Runtime `unavailable`, `timeout`, `error`, `invalid-response`, and `invalid-result` outcomes preserve the current turn and increment an internal consecutive-failure counter. A valid engine action resets the counter. Three consecutive non-busy failures make Zhunshibao resign with an explicit system message so the player is not stranded. Never hide these failures behind a homemade move.
...
- Production defaults to `/usr/games/gnugo`, while local Windows development may set an absolute `gnugo.exe` path. `deploy/update-production.sh` must validate an absolute executable path and a successful `--version` probe before any backup, Git update, migration, or service stop.
...
| GNU Go probe fails | `practice_engine_unavailable` | Keep matchmaking unchanged and do not create a room |
| Valid request and engine probe succeeds | success | Leave matchmaking, create the room, return `roomCode` |
...
- Good: the compact image-backed sibling entry sits immediately to the title's upper-right without covering it, opens a focus-trapped three-level selector, and starts the selected level with random color in one click. All new levels show the red 22-capture opening rule; the bot portrait and frame use ordinary player geometry, and the card reserves missing rating/skill tracks.
- Good: the three tiers invoke GNU Go levels 1, 5, and 10. A bot-visible SGF plus the Spark-legal `restricted_genmove` whitelist produces one revalidated action; a busy slot waits, and an unavailable executable rejects room creation without removing the player from matchmaking.
...
- Bad: adding a color selector or confirmation step, showing the capture-rule line in ordinary games, hardcoding 22 for a restored legacy beginner room, deriving rank outside the difficulty config, using the hidden authoritative board, exposing raw GTP to clients, omitting the Spark legal whitelist, running multiple GNU Go processes, releasing the global slot before a killed child exits, accepting a stale/illegal engine move, silently falling back to homemade heuristics, losing the per-room in-flight key during the engine await, centering the practice entry over the card watermark, covering the Spark title, putting its rotation on the child image where the generic `button > *` reset cancels it, nesting it inside the formal match button, collapsing the no-rating/no-skill bot card shorter than the human card, applying bot-only portrait geometry, creating replay/progression rows, or counting practice rooms as public violates this contract.

## 6. Tests Required

- Socket tests assert all request validation codes, active-room rejection, GNU Go unavailability without matchmaking exit, successful matchmaking exit, and room metadata.
- Home tests assert sibling-button markup, accessible naming, the focus-trapped three-level nested dialog, Escape/focus restoration, selected `{ difficulty, playerColor: "random" }` dispatch, absence of a color selector, exact WebP metadata/position/motion, compact Bright School width, and desktop/mobile owner rules.
...
- Engine adapter tests assert SGF size/komi/player/setup fields, hidden-hand omission, neutral-stone omission, GTP I-column conversion, Spark legal-point restriction, levels 1/5/10, executable probe caching, global busy rejection, timeout/unavailable mapping, and refusal to generate a homemade move.
- Automation tests assert every public tier calls GNU Go, one in-flight action per room/turn, busy retry without a move, explicit resignation after three non-busy failures, all public 22-capture thresholds, legacy 11-capture restore behavior, and ordinary-capture-only semantics.
...
const engineResult = await practiceBotEngine.search(visibleGame, bot.color, difficulty);
if (!engineResult.ok) return retryOrEndExplicitly(engineResult.reason);
if (!isLegalPracticeAction(visibleGame, bot.color, engineResult.action)) {
