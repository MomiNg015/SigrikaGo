...
- Preserve badges, unread indicators and inventory counts; Watch tabs intentionally show only mode names without room counts. Adapt inner grid tracks after portal relocation; keep inner content scrollable and bookmark rails independent. Bound minimum shell height by the safe viewport; labels never wrap: one horizontal line on desktop and one vertical column on phones. Recruitment uses a 148px desktop rail and 16px labels; long phone labels grow naturally and remain reachable through the independent rail scroller. Friends lists align their natural-height rows at the start so sparse lists do not stretch.
