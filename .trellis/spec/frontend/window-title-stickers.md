...
- Preserve badges, unread indicators and inventory counts; Watch tabs intentionally show only mode names without room counts. Adapt inner grid tracks after portal relocation; keep inner content scrollable and bookmark rails independent. Bound minimum shell height by the safe viewport; labels never wrap: one horizontal line on desktop and one vertical column on phones. Recruitment item selection stays inside its action footer as a labelled button group with pressed state; it is not a tab surface and must not opt into bookmarks. Friends lists align their natural-height rows at the start so sparse lists do not stretch.
