...
#### 1. Scope / Trigger
- Trigger: handling full `room:update` snapshots or live `room:resume` snapshots that replace authoritative room state.
...
#### 5. Good/Base/Bad Cases
- Good: `room:update` or live `room:resume` after reconnect can preserve the existing room object when the server sends the same snapshot twice.
...
- Bypassing `applyRoomSnapshot` for full same-room `room:update` or live `room:resume` payloads. This loses structural sharing and makes memoized board/player consumers work harder.
