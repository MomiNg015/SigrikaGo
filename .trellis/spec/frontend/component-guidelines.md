...
- Qiuyuan timing lives in `src/shared/qiuyuanPresentation.js`: main blade starts at 0.24 and sweeps for 0.16. Use the inverse cubic travel for both DOM stone removal and Pixi cut bursts; never interpolate contact times linearly. Scale by the actual board-effect duration and generate fragments only from `removedStones`. The main cut retains the original 1.8-cell ink-smear and broken-white blade artwork and holds until 0.50 before contraction. Avoid solid tapered beam bodies: these read as an energy cannon rather than a sword slash. The DOM scar fades in at 0.50 and settles over 0.20 instead of drawing a second sweep.
