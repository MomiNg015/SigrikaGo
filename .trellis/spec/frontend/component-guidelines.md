...
- Qiuyuan timing lives in `src/shared/qiuyuanPresentation.js`: main blade starts at 0.24 and sweeps for 0.16. Use the inverse cubic travel for both DOM stone removal and Pixi cut bursts; never interpolate contact times linearly. Scale by the actual board-effect duration and generate fragments only from `removedStones`. The main cut uses a broad tapered 2.8-cell ink blade, distinct from the thin omens, and holds until 0.50 before contraction. The DOM scar fades in at 0.50 and settles over 0.20 instead of drawing a second sweep.
