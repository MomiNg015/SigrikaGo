...
- Qiuyuan timing lives in `src/shared/qiuyuanPresentation.js`: main blade starts at 0.24 and sweeps for 0.16. Use the inverse cubic travel for both DOM stone removal and Pixi cut bursts; never interpolate contact times linearly. Scale by the actual board-effect duration and generate fragments only from `removedStones`. The DOM scar fades in at 0.40 and settles over 0.20 instead of drawing a second sweep.
