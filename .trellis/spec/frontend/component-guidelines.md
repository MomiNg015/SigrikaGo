...
- `src/shared/sigrikaPresentation.js` owns Sigrika `erase-point` contact progress `0.58`, Sunspirit reveal progress `0.76`, and the 200ms visual invalid-region reveal duration; the sound catalog, `boardSigrikaEffect.js`, and `Board` consume this shared phase instead of duplicating timing literals.
...
- Board visuals that need only persistent DOM/CSS markers may keep `SKILL_EFFECT_CATALOG[effectType].boardEffect === false`; `BoardSkillEffects` then returns `null` for those active previews so no full-board overlay layer or Pixi canvas can cover the grid. QiuYuan `row-slash` is the exception: it uses a short full-board Pixi cast for transient white/teal ink-blade motion, while `BoardRowSlashOverlay` owns the temporary row scar and dissolves it before resolution.
- Qiuyuan Pixi and DOM scar layers belong to the outer `.board-wrap`, not the inner `.board` grid. Keep clipping at the wooden surface edge; measure actual grid geometry for cell size and row position. The DOM scar tracks grid offset and height via ResizeObserver and disconnects on cleanup. Render it only during pending row-slash: dissolve from progress 0.64 to 0.96 before the default 4000ms resolution; never replay resolved rowEffects. Preserve the original strike artwork.
...
- Sigrika `erase-point` is a falling star revealing a sleepy Sunspirit (日灵), not a crater. Keep the legacy `/assets/effects/sigrika-erased-field-marker.webp` URL and 150% point-cell DOM marker size. The isolated `boardSigrikaEffect.js` renderer uses cell-scaled geometry, a trajectory-aligned tapered trail, a short target rune, a dedicated gold-white contact burst before the delayed mint Sunspirit reveal, and sparse returning stardust; do not draw transient crater/scorch residue.
- The Sunspirit's small settling motion uses individual `translate` / `scale` properties because Bright School's existing `button > *` guard forces `transform: none !important`. Reduced motion resets both individual properties. Its reduced boundary selectors include `.erased-boundary-layer` so the later `grid-scoring.css` animation shorthand cannot restore the full-motion contact delay; browser computed marker and boundary delays must both equal the banner duration in reduced mode.
- Sigrika star contact, impact SFX, burst and visual invalid-region reveal share contact progress `0.58`; the Sunspirit stays hidden during the burst and appears at `0.76` (324ms later on the default timeline). `Board` crossfades existing and pending invalid-region geometry over 200ms starting at contact, then hands off to authoritative geometry at resolution; adjacent existing invalid points must keep correct boundaries. This is presentation-only: preserve point eligibility, game state, 2000ms banner, 1800ms board effect, and 4000ms settlement. Reduced motion keeps the static hit with no SFX and reveals marker/boundary at banner end, while the all-effects-off gate still suppresses pending presentation.
...
- Sigrika tests assert shared contact timing across catalog/Pixi/DOM, no impact feedback before contact, transparent transient graphics at completion, cell-relative geometry at center/corners, and a visual invalid-region handoff that preserves existing adjacent boundaries and point eligibility; verify reduced-motion and effects-disabled paths.
...
- Self-resume achievement and personalization actions use `identityActions`, absolutely anchored to the identity card's lower right without altering username layout. These and replay actions in both self and social profiles are circular, shadowless icons with accessible labels. Preserve the original pink/blue/green palette through local shared-button variables; retain battle replay disabling and other social actions.
