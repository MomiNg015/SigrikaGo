...
- `CSS_DEBT_BASELINE` is the 2026-07-03 all-`src/styles` non-growth baseline for CSS file count, bytes, `!important`, hardcoded hex values, media-query files, reduced-motion files, and high z-index files. It includes the current hidden player-window scrollbar, mobile music shop card, left-aligned replay-time, and story-player padding contracts. Cleanup should reduce these counts or document why a contract update is necessary. Do not treat the baseline as permission to add visual drift.
...
- Current Phase 4 pilot: `src/modals/modalComponents.jsx` wraps `Button` as `ModalActionButton`; `src/modals/FeedbackModals.jsx` uses it for `ConfirmModal` danger/secondary actions, `src/modals/MessageBoardModal.jsx` uses it for the submit action, `src/modals/AnnouncementModal.jsx` uses it for simple retry/load-more secondary actions, `src/modals/PersonalizationModal.jsx` uses it for the save action, `src/modals/MailboxModal.jsx` uses it for the attachment claim action, `src/modals/friends/FriendsOverlays.jsx` uses it for the duel-mode cancel action, and `src/modals/UserProfileCard.jsx` uses it for the profile report submit action. This moves only action alignment utilities through the primitive layer; existing modal/action, mailbox, and friends/profile CSS still owns colors, spacing, borders, shadows, typography, disabled states, placement, list/detail layout, paper background, match-mode option buttons, profile confirm panels, profile social actions, and theme/mobile overrides.
