...
- `src/styles/styleContract.test.js` also owns the `mobile-adaptive.css` import order plus the nested `home-narrow-desktop.css`, `bright-school-overrides.css`, and `bright-school-portrait.css` import orders because these entries are the final safety layers after theme imports.
