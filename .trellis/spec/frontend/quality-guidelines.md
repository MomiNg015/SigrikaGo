...
#### 1. Scope / Trigger
- Trigger: changing the Sigrika corruption root class, home controls, handbook cards, match picker, special result continuation, the development candy-badge cancel action, or the development story jump to use eight.
...
- The special room renders an unlimited digital clock with damaged-number treatment. It does not expose skill, draw, chat, watch, join, or ordinary leave-as-resign paths.
- The result surface cannot be dismissed around recovery. Its only continuation starts the win/loss recovery story; finishing or skipping that story restores the normal app shell.
...
- Bad: point the option directly at `corruption-start` without a server write, expose the option in production, or replace the user with a relation-incomplete projection that resets equipped portraits.

#### 6. Tests Required
- Home, handbook, match, IRIS/footer, story, result, room, timer, and replay suites assert both markup restrictions and semantic controls.
...
- CSS/static contracts assert the final corruption stylesheet is imported after normal theme/mobile layers, keeps each concrete owner below the size guard, imports the home/handbook/match/theme-guard owners in order, and owns the deliberate no-reduced-motion exception.
