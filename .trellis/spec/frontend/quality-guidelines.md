...
#### 1. Scope / Trigger
- Trigger: changing the Sigrika corruption root class, official corrupted portrait, home controls, handbook cards, match picker, special-room player/presentation surfaces, result continuation, the development candy-badge cancel action, or the development story jump to use eight.
...
- Special duel room/replay -> the NPC card and fake-skill burst contain the canonical corrupted-Sigrika WebP, the human card contains the canonical anonymous-player WebP, and battle preload contains both WebPs even when `skillEnabled` is false. The room root and each direct-child presentation portal compute `filter: none`; dialogue and fake-skill banner each expose exactly one continuous outer background, while their portrait/copy children remain borderless and transparent. Header, action and system-record Lucide icons compute a pale high-contrast stroke, with the disabled counting glyph retaining its muted disabled stroke. Both information rows contain all three Go counters, the room code is `房间号ERROR`, and counting remains disabled. Computed black/white portrait fills are distinct; the NPC skill chip has no cream background image. Opening/confirmation compute at `460px`/`420px` maximum width with hidden internal overflow and readable pale action labels. The system log contains no `npc-thinking` line. The special replay row shows an icon-only boss badge and pale blood-red background, uses the canonical NPC/player portraits, and keeps the timestamp first with no special title/tag prefix. The special match-success window says `正在踏入深渊。。。`; the ordinary window still says `匹配成功`. At 1280×720 the action bar is fully visible without changing the timer contract; 390×844 and 412×915 have no horizontal document overflow. Confirmed resignation produces the result without early navigation, and that result has no record disclaimer, no result voice event, and exactly one continuation.
...
- Corrupted home -> `resolveBackgroundMusic()` returns the internal `sigrika-corruption-home` single-loop before consulting the random home pool; `SIGRIKA_CANDY_DUEL.matchSource` -> returns the internal duel intro-loop before ordinary battle or skill music. Match-success, result-modal, and finished-room silence gates stay higher priority. Recovery to a normal user payload restores the ordinary resolver. The visual corruption overlay must not acquire the shared BGM pause request; character-music preview and other explicit pause consumers keep the existing offset-preserving pause contract.
...
- Bad: point the option directly at `corruption-start` without a server write, expose the option in production, replace the user with a relation-incomplete projection that resets equipped portraits, duplicate the portrait URL across components, or let an equipped costume override the corrupted portrait.

#### 6. Tests Required
- Home, handbook, match, IRIS/footer, story, result, room, timer, and replay suites assert both markup restrictions and semantic controls. Special-room coverage locks the abyss match-success heading, invisible-only unlimited-time accessible name, visible three-counter row, disabled counting action, `房间号ERROR`, absent result disclaimer, null result voice event, real opening/confirmation atmosphere child, ordinary-room modal isolation, and resignation-without-early-navigation behavior. CSS contracts lock the dedicated icon owner, pale/disabled stroke roles, one continuous presentation background, and transparent borderless portrait/copy children.
...
- `SigrikaCorruptionOverlay.dom.test.jsx` asserts the pointer-transparent damage structure without acquiring the global BGM pause; `musicLibrary.test.js` locks corrupted-home and special-duel resolver priority, while `preloadAssets.test.js` locks conditional home preload and match-source duel preload.
