...
- Corruption mounts while BGM is active -> preserve offset and stop playback; background track changes before recovery -> replace the pending track but schedule no source; corruption unmounts -> resume the pending track from its stored offset contract.
...
- `SigrikaCorruptionOverlay.dom.test.jsx` asserts the pointer-transparent damage structure and the complete acquire/release BGM pause lifetime.
