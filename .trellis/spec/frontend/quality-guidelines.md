...
- On desktop and portrait screens, `.profile-character-table-head` is the visible six-column header outside `.profile-character-table-scroll`; it shares the table's columns and reserves the row scroller's native gutter through non-scroll layout spacing. The header itself stays `overflow: hidden` with zero scroll range, transparent/borderless/shadowless, bottom-aligns “角色战绩” with the five column labels, and never uses sticky positioning. The semantic table `thead` remains in the DOM but is visually collapsed so screen-reader semantics survive without duplicating the visible header. `.profile-character-table-scroll` owns only the table body's vertical overflow inside the fixed-height dossier on both desktop and portrait screens.
