...
- Bright School portrait handbook art must use the direct-child selector `.character-card.portrait-card > img`, never resize nested `24px` item badges. Ordinary `.campus-handbook:not(.is-sigrika-corrupted)` uses two columns, a separate name/action row and a `44px` flag button; its artwork fits the remaining portrait row. The earlier final-mobile `72px` art / `30px` absolute action remains the fallback outside that ordinary handbook scope. Confirm real computed layout at 320px and 390px, including full four-character names, non-overlapping art/actions, and roster scrolling.
