...
export function scorePracticeMove(gameView, botColor, pointId) {
  ...
  for (const neighborPoint of neighbors) {
    ...
    if (neighborPoint.stone === botColor) {
      ...
      if (group.liberties.size === 2 && group.liberties.has(pointId)) {
        threatenedEnemies += group.stones.length;
      }
    }
  }

  const result = playMove(gameView, botColor, pointId, { colorIllusion: null });
  ...
  const shape = localShapeScore(next, botColor, pointId);
  ...
  return { pointId, score, priority, reasons };
}
...
function emptyTriangleCount(game, color, point) {
  ...
  for (const offsetX of [-1, 0]) {
    for (const offsetY of [-1, 0]) {
      const block = coordinatePoints(game, { x: point.x + offsetX, y: point.y + offsetY }, [[0, 0], [1, 0], [0, 1], [1, 1]]);
