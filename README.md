...
先通过公开注册创建普通账号，再在服务器项目目录执行本地管理员提升命令：

```powershell
npm run admin:promote -- alice
```

命令只会提升数据库中已存在的账号；账号不存在时会失败，不会代替用户注册。重复提升同一账号是安全的。管理员登录后可以从大厅进入后台管理，注册、登录、刷新和服务启动过程都不会依据用户名自动授予管理员权限。
